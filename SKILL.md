...
# wechat-mp-article-push · 微信公众号图文生成与推送技能
...
| 项          | 内容                                                                                                                         |
| ---------- | -------------------------------------------------------------------------------------------------------------------------- |
| **配置向导地址** | [https://app.pcloud.ac.cn/design/wechat-mp-article-push.html](https://app.pcloud.ac.cn/design/wechat-mp-article-push.html) |
...
首参为 `img`，第二参为**图片链接的 JSON 数组字符串**（整段一个参数；Bash 与 PowerShell 都可用单引号包住整段 JSON，例如 `'["https://...","https://..."]'`）。再依次传标题、正文、`sendMode`（可选）。
